...
qc_plus= read.table(file.path(path_results, "basic-bam", "flagstat.tsv"), header=T, sep= "\t", check.names = T, fill=T)
...
```{r read-qual, eval=FALSE, echo=FALSE}
ggplot(qual %>% filter(type=="nt_qual"), aes(x=pos, y=mean_Q, group=Fixed_name, colour=vendor)) +
    geom_line() +
    facet_wrap(~sample) +
    ylim(0,41) +
...
```{r read-size, eval=FALSE, echo=FALSE}
ggplot(qual %>% filter(type=="length"), aes(x=pos, y=mean_Q, group=Fixed_name, colour=vendor)) +
...
```{r read-content, eval=FALSE, echo=FALSE}
qual = read.table("~/orch/scratch/merck_5_vendors/vendors/final-validate/2015-04-01_vendors/fastqc_content.tsv", header=T, sep= "\t", check.names = F)
names(qual) = c("type", "Name", "pos", "pos_str", "G", "A", "T", "C")
qual$Fixed_name = fix_names(as.character(qual$Name))
qual$sample = get_samples(as.character(qual$Fixed_name))
qual$vendor = get_vendor(as.character(qual$Name))
library(gtools)
qual$pos = factor(qual$pos, levels=mixedsort(unique(qual$pos)))

for (nt in c("G", "A", "T", "C")){
    pos = "pos"
    Fixed_name = "Fixed_name"
    vendor = "vendor"
    p = ggplot(qual %>% filter(type=="content"), aes_string(x=pos, y=nt, group=Fixed_name, colour=vendor)) +
        geom_line(size=2, alpha=0.5) +
        facet_wrap(~sample) +
        scale_color_brewer(palette = "Set1") +
        theme_bw() +
...
# kable(tab)
tab$Fixed_name = fix_names(as.character(tab$sample))
tab$sample = get_samples(as.character(tab$Fixed_name))
tab$vendor = get_vendor(as.character(tab$sample))
...
### variants coverage in common regions
...
### variants coverage vs CG content in common regions
...
make_quantile_plots = function(cov_tab){
    p1 = ggplot(cov_tab %>% filter(min_reads=='q10'), aes(x=type, y=quantiles, colour=sample, group=sample)) +
    geom_line(size=2, alpha=.5)+
    theme_bw()+
    labs(list(x="% of target regions with more than X bases covered", y="% of nt covered inside the target", title="considered covered when nt has >10 reads"))

    p2 = ggplot(cov_tab %>% filter(min_reads=='q25'), aes(x=type, y=quantiles, colour=sample, group=sample)) +
    geom_line(size=2, alpha=.5)+
    theme_bw()+
    labs(list(x="% of target regions with more than X bases covered", y="% of nt covered inside the target", title="considered covered when nt has >25 reads"))

    p3 = ggplot(cov_tab %>% filter(min_reads=='q50'), aes(x=type, y=quantiles, colour=sample, group=sample)) +
    geom_line(size=2, alpha=.5)+
    theme_bw()+
    labs(list(x="% of target regions with more than X bases covered", y="% of nt covered inside the target", title="considered covered when nt has >50 reads"))
...
make_bias_plot = function(cov_tab){
    p1 = ggplot(cov_tab, aes(x=log2(mean), y=cv, colour=sample)) +
    ...
    p2 = ggplot(cov_tab, aes( x=sample,y=bias, colour=sample)) +
    geom_jitter(alpha=0.5) +
    scale_color_brewer(palette = "Set1") + 
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
    labs(list(y="% of nt with mean-2SD > coverage > mean+2SD ")) +
        ggtitle("% of nucleotides with extreme coverage inside target regions")
