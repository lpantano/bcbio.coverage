...
# Import/concat BED coverage files
file_list <- list.files(path=file.path(path_results, "coverage"), pattern='*_coverage_fixed.bed')
...
for (file in file_list){
  ...
  if (!exists("dataset")){
    dataset <- read.table(file.path(path_results, 'coverage', file), header=FALSE, 
...
# 20x cutoff
cutoff <- dataset %>% dplyr::select(region, sampleName, percentage20) %>%  distinct(region) %>% 
